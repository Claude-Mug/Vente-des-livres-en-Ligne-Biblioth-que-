...
// Initialiser $admin à null pour éviter l'erreur "variable non définie"
$admin = null;

if (!empty($admins)) {
    // Sélectionner le premier administrateur (ou un autre critère)
    $admin = $admins[0]; // Ici, on prend le premier administrateur de la liste
    $idAdmin = $admin['idAdmin']; // Récupérer l'ID de l'administrateur
    ...
    die("Aucun administrateur trouvé dans la base de données.");
}
...
<body>
    <div class="container">
        <h1 class="mt-4">Profil Administrateur</h1>
        ...
        <?php if ($admin): ?> <!-- Vérifier si $admin est défini -->
            <div class="card mt-4">
        ...
        <?php else: ?>
            <div class="alert alert-danger">Aucun administrateur trouvé.</div>
        <?php endif; ?>
    </div>
</body>
</html>